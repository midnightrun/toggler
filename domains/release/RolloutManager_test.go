	...
	s.When(`it had been persisted previously`, func(s *testcase.Spec) {
		...
		s.And(`there are pilots manually set for the feature`, func(s *testcase.Spec) {
			...
			var getPilotCount = func(t *testcase.T) int {
				count, err := iterators.Count(ExampleStorage(t).FindAll(GetContext(t), release.ManualPilotEnrollment{}))
				require.Nil(t, err)
				return count
			}
			...
			s.And(`if other flags have pilots enrolled as well`, func(s *testcase.Spec) {
				...
				s.Around(func(t *testcase.T) func() {
					...
					return func() {
						require.Nil(t, ExampleStorage(t).DeleteAll(GetContext(t), release.ManualPilotEnrollment{}))
					}
				})
	...
	s.When(`pilot already exists`, func(s *testcase.Spec) {
		...
		s.And(`and pilot is has the opposite enrollment status`, func(s *testcase.Spec) {
			...
			s.Then(`the original pilot is updated to the new enrollment status`, func(t *testcase.T) {
				...
				pilot, err := ExampleStorage(t).FindReleasePilotByReleaseFlagAndDeploymentEnvironmentAndExternalID(
				...
				expectedPilot := release.ManualPilotEnrollment{
				...
				count, err := iterators.Count(ExampleStorage(t).FindAll(context.Background(), release.ManualPilotEnrollment{}))
				require.Nil(t, err)
		...
		s.And(`pilot already has the same enrollment status`, func(s *testcase.Spec) {
			...
			s.Then(`pilot remain the same`, func(t *testcase.T) {
				...
				pilot, err := ExampleStorage(t).FindReleasePilotByReleaseFlagAndDeploymentEnvironmentAndExternalID(context.Background(), ff.ID, ExampleDeploymentEnvironment(t).ID, ExampleExternalPilotID(t))
				require.Nil(t, err)
	...
	s.When(`flag already persisted`, func(s *testcase.Spec) {
		...
		s.And(`pilot already exists`, func(s *testcase.Spec) {
			...
			s.Then(`pilot manual enrollment will be removed`, func(t *testcase.T) {
				...
				pilot, err := ExampleStorage(t).FindReleasePilotByReleaseFlagAndDeploymentEnvironmentAndExternalID(context.Background(), flag.ID, "", ExampleExternalPilotID(t))
				require.Nil(t, err)
				require.Nil(t, pilot)
			})
		})
	...
	s.When(`flag and rollout plan is present`, func(s *testcase.Spec) {
		...
		s.And(`by rollout plan`, func(s *testcase.Spec) {
			s.Context(`it is not participating`, func(s *testcase.Spec) {
				AndReleaseFlagPercentageIs(s, ExampleReleaseRolloutLetVar, 0)
			...
			s.Context(`it is participating`, func(s *testcase.Spec) {
				AndReleaseFlagPercentageIs(s, ExampleReleaseRolloutLetVar, 100)
